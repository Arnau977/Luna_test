...
// TODO entry.executor no se puede leer executor de null
...
// Intervals
setInterval(Name_Reminder, 2 * 60 * 1000); // Logs the tag of the bot in console
setInterval(update_commands, 30 * 60 * 1000); // Update the commands directory to see changes and add/delete the commands
...
client.on('disconnect', async () => {
  autorUser.send("He sido desconectado");
});
...
client.on('message', async message => {
  ...
  if (message.content.startsWith(cfg.PREFIX)) {
    ...
    if (message.author.id === autorUser.id && commandName === "restart") {
      RestartBot();
      return;
    }
...
function createDB(guild_id = '680576022608871462') {
...
function deleteDB(guild_id = '680576022608871462') {
