    ...
    execute(message = Discord.Message, args = '') {
        try {
            ...
            pool.query(SQLIST, function (err, Pelisresult) {
                ...
                var SQLIST;
                if (args) {
                    lowerArg = args.toLocaleLowerCase();
                    if (lowerArg == 'abc') {
                        SQLIST = `SELECT Nombre_Pelicula, estado.FechaVisualización, estado from peliculas join estado on estado.id_pelicula = peliculas.id_pelicula where estado like 'Vista' order by Nombre_Pelicula;`;
                    } else if (lowerArg == 'fecha' || lowerArg == 'date') {
                        SQLIST = `SELECT Nombre_Pelicula, estado.FechaVisualización, estado from peliculas join estado on estado.id_pelicula = peliculas.id_pelicula where estado like 'Vista' order by estado.FechaVisualización;`;
                    }
